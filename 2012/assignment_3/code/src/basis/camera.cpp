...
void Camera::ArcBallRotation(int x, int y)
{
    ...
    sl = (float)hypot(sx, sy);
    el = (float)hypot(ex, ey);
...
void Camera::PlaneTranslation(int x, int y)
{
    ...
    float d = float(mViewport[3])/2.0f / (float)FW::tan(mPerspective[0]*M_PI / 180.0f / 2.0f);
...
{
	// back up distance
    Mat4f tmp = lookat( Vec3f(0, 0, mCurrentDistance),
                        Vec3f(0, 0, 0),
                        Vec3f(0, 1, 0));

	return tmp * mCurrentRot * translation( -mCurrentCenter );
}
