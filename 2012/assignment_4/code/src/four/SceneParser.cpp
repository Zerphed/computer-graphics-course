...
Material* SceneParser::parseMarble(int count)
{
    ...
    int amplitude = readInt();
	assert(amplitude > 0);

	getToken(token); assert (!strcmp(token, "}"));

    return new Marble(matrix, materials[m1], materials[m2], octaves,
		frequency, amplitude);
}
...
Box* SceneParser::parseBox() {
	...
	getToken( token ); assert (!strcmp(token, "near"));
    Vec3f nearp = readVec3f();

    getToken( token ); assert (!strcmp(token, "far"));
    Vec3f farp = readVec3f();

    getToken( token ); assert (!strcmp(token, "}"));
    assert (current_material != NULL);
	return new Box(nearp, farp, current_material);
}
